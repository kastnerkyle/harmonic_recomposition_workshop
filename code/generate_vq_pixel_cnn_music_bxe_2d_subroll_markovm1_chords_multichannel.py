...
with tf.Session(config=config) as sess1:
    ...
    cond_z = np.zeros((1, pix_z.shape[1], pix_z.shape[2]))
    for k in np.arange(0, pix_z.shape[0]):
        temp_z = np.zeros((1, pix_z.shape[1], pix_z.shape[2]))
        y_sub = sample_labels[k][None]
        for i in range(pix_z.shape[1]):
            for j in range(pix_z.shape[2]):
                print("Sampling step {} completion pixel {}, {}".format(k, i, j))
                feed = {vs.images: temp_z[..., None],
                        vs.conds: cond_z[..., None],
                        vs.labels: y_sub}
                outs = [vs.x_tilde]
                r = sess1.run(outs, feed_dict=feed)
                x_rec = sample_gumbel(r[-1])
                temp_z[0, i, j] = x_rec[0, i, j]
        pix_z[k] = temp_z[0]
        cond_z[0] = temp_z[0]
